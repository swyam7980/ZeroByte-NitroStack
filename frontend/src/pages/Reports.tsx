...
export function Reports() {
  const { confirmedFindings, scan, riskScore, report, reportLoading } = useStore();
            ...
            <div className="p-3 border-b border-outline-variant flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-label-md text-label-md text-on-surface uppercase tracking-wider">
                  Rendering from Reporting MCP
                </span>
              </div>
              <span className="font-mono-code text-[11px] text-on-surface-variant">
                {reportLoading ? "Loading…" : report ? `${report.score}/100` : "Pending"}
              </span>
            </div>
            <ReportView html={report?.html ?? null} loading={reportLoading} />
          </div>
        </div>
